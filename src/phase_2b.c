...
void reEncodeSequence_Phase2b (PROG_INFO *prog_struct, BLOCK_INFO *block_struct, R_UINT block_id) {
  R_UINT x = 0;
  R_UINT y = 0;
  R_UINT z = 0;
  R_UINT bytes_read;
  ...
  **  and distance between synchronization points  */
  ...
  if (block_id == 1) {
    ...
    fprintf (stderr, "Block id incorrect in calling reEncodeSequence_Phase2b in [%s, %u].\n", __FILE__, __LINE__);
  }
  ...
  char_buffer[MAX_CHAR_BUFFER].ch = UINT_MAX;
  ...
  while (R_TRUE) {
	...
	segment_start = 0;
      ...
      for (i = 0; i < phrase_len; i++) {
        ...
        char_buffer[segment_end].ch = target_phrases[y].str[i];
    ...
    **  block marker  */
...
#ifdef DEBUG
        fprintf (stderr, "Segment summary:\n");
        fprintf (stderr, "\t%u segments in total:\n", segment_count);
        fprintf (stderr, "\t%u primitives in total\n", total_segment_length);
        fprintf (stderr, "\tAverage length of a segment:  %f\n\n", (R_FLOAT) total_segment_length / (R_FLOAT) segment_count);
#endif
	...
	if ((check_position + curr_phrase -> len) > max_position) {
      ...
      if ((check_position == max_position) && (eob_found != -1)) {
        total_segment_length += (max_position - segment_start);
        segment_count++;
